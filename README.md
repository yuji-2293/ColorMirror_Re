...
- [CI/CD設計](#cicd設計)
- [テスト設計](#テスト設計)
- [これにより、ブラウザからDB保存までの主要フローを通貫してCI上で検証できる環境を構築しました。](#これによりブラウザからdb保存までの主要フローを通貫してci上で検証できる環境を構築しました)
- [開発背景](#開発背景)
...
- frontend / backend / E2E のworkflowを分離
  - モノレポ構成に合わせて、変更されたディレクトリ単位でworkflowを制御
    ...
    - front/backどちらかに変更があれば、e2e.ymlが走る
...
Unit Test:
- API
- Hooks(TanStack Queryを利用したServer State処理を含む)
- Components
- zustand(store)
- Validation
- Routing Guard
...
実行環境: Chromium１本に絞り、テスト実行時間を短縮しました。 
