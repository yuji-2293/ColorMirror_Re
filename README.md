...
## アプリ画像
<p>
  <img src="front/App_front/public/assets/topImage.png"　width="600" height="300" />
</p>
...
## 既存ポートフォリオをReactのモノレポ構成でリプレイスしてみた所感
### [Rails(turbo) + JS] *→*  [React + vite + TypeScript + Rails(APIモード)]
...
## 開発背景
  ...
  ### JavaScript / Rails[turbo]での開発で感じた課題
    ...
    → これらを直線的な体験として繋ぐ構成となっていた。  
      ...
      これらを1つの画面に複合させる必要があった。  
  
      結果として、自然とアプリのディレクトリ構成がコンポーネントライクに寄せてしまい、UIを担うのがJavaScriptとRails(turbo)との間で曖昧になってしまっていることが***課題***だった。  

  ### なぜReact + Rails API構成にしたのか
      上記の課題により、フロントとサーバーとで責務を分割して、複合的になっているUI層を整理した形でアプリを作ってみたいと考え***React***を採用することにした。  
